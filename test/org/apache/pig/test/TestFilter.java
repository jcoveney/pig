...
public class TestFilter {
    ...
    public void testSimpleFilter() throws Exception {
        for (int i = 0; i < nullFlags.length; i++) {
            ...
            assertEquals("Running " + getClass().getName() + "with nullFlags set to "+ nullFlags[i] + ":", POStatus.STATUS_EOP, res.returnStatus);
            assertEquals("Running " + getClass().getName() + "with nullFlags set to "+ nullFlags[i] + ":", 1, outbag.size());
            Iterator<Tuple> it = outbag.iterator();
            assertTrue("Running " + getClass().getName() + "with nullFlags set to "+ nullFlags[i] + ":", it.hasNext());
            t = it.next();
            assertEquals("Running " + getClass().getName() + "with nullFlags set to "+ nullFlags[i] + ":", 2, t.size());
            assertTrue("Running " + getClass().getName() + "with nullFlags set to "+ nullFlags[i] + ":", t.get(0) instanceof Integer);
            assertTrue("Running " + getClass().getName() + "with nullFlags set to "+ nullFlags[i] + ":", t.get(1) instanceof Integer);
            Integer i1 = (Integer)t.get(0);
            Integer i2 = (Integer)t.get(1);
            assertEquals("Running " + getClass().getName() + "with nullFlags set to "+ nullFlags[i] + ":", 1, (int)i1);
            assertEquals("Running " + getClass().getName() + "with nullFlags set to "+ nullFlags[i] + ":", 0, (int)i2);
        }
    }
