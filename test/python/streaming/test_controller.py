...
class TestCastVal( unittest.TestCase ):
    def test__chararray(self):
        input = "abcde"
        expected_output = "abcde"
        out = controller.cast_val(input, 'chararray', 0, 4)
        self.assertEquals(expected_output, out)

    def test__long(self):
        input = "970916083725"
        expected_output = 970916083725L
        out = controller.cast_val(input, 'long', 0, 11)
        self.assertEquals(expected_output, out)

    def test__empty_string(self):
        input = ""
        expected_output = ""
        out = controller.cast_val(input, 'chararray', 0, -1)
        self.assertEquals(expected_output, out)

    def test__one_char_string(self):
        input = "a"
        expected_output = "a"
        out = controller.cast_val(input, 'chararray', 0, 0)
        self.assertEquals(expected_output, out)

    def test__empty_long(self):
        input = ""
        expected_output = None
        out = controller.cast_val(input, 'long', 0, -1)
        self.assertEquals(expected_output, out)

    def test__mid_int(self):
        input = "1,abc,99,def"
        expected_output = 99
        out = controller.cast_val(input, 'int', 6, 7)
        self.assertEquals(expected_output, out)
...
class TestReadInput( unittest.TestCase ):
    def test__multiline_record(self):
        ...
        inputio.write('5|&\n')
        inputio.seek(0)
        out = controller.get_next_input(sys.stdout, inputio)
