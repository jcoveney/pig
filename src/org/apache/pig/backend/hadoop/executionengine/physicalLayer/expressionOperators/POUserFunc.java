...
public class POUserFunc extends ExpressionOperator {
    ...
    private static ThreadLocal<Boolean> haveSeenJobConfNotAvailableWarning = new ThreadLocal<Boolean>() {{
      set(false);
    }};
    ...
    public Result processInput() throws ExecException {
        ...
        if(!initialized) {
            ...
            if (jobConf != null) {
                doTiming = jobConf.getBoolean(TIME_UDFS_PROP, false);
                counterGroup = funcSpec.toString();
            } else {
                if (!haveSeenJobConfNotAvailableWarning.get()) {
                    LOG.warn("jobConf not available. Not tracking UDF timing regardless of user preference.");
                    haveSeenJobConfNotAvailableWarning.set(true);
                }
            }
