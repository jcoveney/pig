...
//        System.err.println("Exiting Store");
    ...
    public void visit(LOJoin loj) throws FrontendException {
        ...
        if (loj.getJoinType() == LOJoin.JOINTYPE.SKEWED) {
            ...
            // innerFlags[0] refers to the RIGHT operator.
            boolean usePOMergeJoin = inputs.size() == 2 && innerFlags[0] ;
            System.err.println("Use MergeJoin? " + usePOMergeJoin + " inp size? "
            + inputs.size() + "innerFlags? " + Arrays.toString(innerFlags));
            if(usePOMergeJoin){
                ...
                try {
                    System.err.println("MG: left relation schema is: " + ((LogicalRelationalOperator)inputs.get(0)).getSchema());
                    System.err.println("MG: right relation schema is: " + ((LogicalRelationalOperator)inputs.get(1)).getSchema());
