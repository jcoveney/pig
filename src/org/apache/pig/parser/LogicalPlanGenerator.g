...
flatten_generated_item returns[LogicalExpressionPlan plan, FlattenStates flattenFlag, LogicalSchema schema]
@init {
    $plan = new LogicalExpressionPlan();
    $flattenFlag = FlattenStates.DO_NOTHING;
}
 : ( flatten_clause[$plan] { $flattenFlag = FlattenStates.FLATTEN_WITH_PREFIX; }
   | col_range[$plan]
   | expr[$plan] { $flattenFlag = $expr.flattenFlag; }
   | STAR
     {
...
func_eval[LogicalExpressionPlan plan] returns[LogicalExpression expr, FlattenStates flattenFlag]
@init {
 ...
 : ^( FUNC_EVAL func_name ( real_arg[$plan] { args.add( $real_arg.expr ); } )* )
   {
       ...
       Pair<LogicalExpression,FlattenStates> pair = builder.buildUDF( loc, $plan, $func_name.funcName, args );
       $expr = pair.getFirst();
       $flattenFlag = pair.getSecond();
   }
;
...
expr[LogicalExpressionPlan plan] returns[LogicalExpression expr, FlattenStates flattenFlag]
@init {
    $flattenFlag = FlattenStates.DO_NOTHING;
}
 ...
 | var_expr[$plan]
   {
       $expr = $var_expr.expr;
       $flattenFlag = $var_expr.flattenFlag;
   }
 | ^( NEG e = expr[$plan] )
   {
...
var_expr[LogicalExpressionPlan plan] returns[LogicalExpression expr, FlattenStates flattenFlag]
@init {
    List<Object> columns = null;
    SourceLocation loc = new SourceLocation( (PigParserNode)$var_expr.start );
    $flattenFlag = FlattenStates.DO_NOTHING;
}
 : projectable_expr[$plan] {
       $expr = $projectable_expr.expr;
       $flattenFlag = $projectable_expr.flattenFlag;
   }
   ( dot_proj
     {
...
projectable_expr[LogicalExpressionPlan plan] returns[LogicalExpression expr, FlattenStates flattenFlag]
@init {
    $flattenFlag = FlattenStates.DO_NOTHING;
}
 : func_eval[$plan]
   {
       $expr = $func_eval.expr;
       $flattenFlag = $func_eval.flattenFlag;
   }
 | col_ref[$plan]
   {
       $expr = $col_ref.expr;
...
@init {
    ...
    List<LogicalExpressionPlan> plans = new ArrayList<LogicalExpressionPlan>();
    List<FlattenStates> flattenFlags = new ArrayList<FlattenStates>();
