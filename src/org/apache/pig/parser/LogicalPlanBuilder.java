...
import org.apache.pig.builtin.FlattenOutput;
...
public class LogicalPlanBuilder {
	...
	List<FlattenStates> flattenFlags = Lists.newArrayList();
	for (int idx = 0; idx < allExprPlan.size(); idx++) {
		...
		if (oper instanceof ProjectExpression) {
		    flattenFlags.add(FlattenStates.DO_NOTHING);
		} else if (oper instanceof UserFuncExpression) {
		    flattenFlags.add(FlattenStates.FLATTEN_WITH_PREFIX); //TODO(jco) do we need to introspect the UserFunc?
		}
	    }
	}
    ...
    void buildGenerateOp(SourceLocation loc, LOForEach foreach, LOGenerate gen,
            ...
            List<LogicalExpressionPlan> exprPlans, List<FlattenStates> flattenFlags,
    ...
    Pair<LogicalExpression,FlattenStates> buildUDF(SourceLocation loc, LogicalExpressionPlan plan,
            String funcName, List<LogicalExpression> args)
    throws RecognitionException {

        FlattenStates flattenFlags = FlattenStates.DO_NOTHING;
        Class<?> func;
        try {
            func = pigContext.getClassForAlias(funcName);
            FunctionType.tryCasting(func, FunctionType.EVALFUNC);
            FlattenOutput annotation = func.getAnnotation(FlattenOutput.class);
            if (annotation != null) {
                if (annotation.prefix()) {
                    flattenFlags = FlattenStates.FLATTEN_WITH_PREFIX;
                } else {
                    flattenFlags = FlattenStates.FLATTEN_WITHOUT_PREFIX;
                }
            }
        } catch (Exception e) {
            throw new PlanGenerationFailureException(intStream, loc, e);
        ...
        le.setLocation(loc);
        return Pair.make(le, flattenFlags);
    }
    ...
    throws ParserValidationException {
        ...
        FlattenStates[] flatten = new FlattenStates[exprPlans.size()];
